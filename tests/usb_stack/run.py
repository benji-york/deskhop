#!/usr/bin/env python3
"""Compile real TinyUSB device/control/HID/MSC stacks against a virtual DCD."""
import os
...
APP_SOURCES = [ROOT / "src/usb_descriptors.c", ROOT / "src/usb.c"]
STACK_SOURCES = [TINYUSB / source for source in ["tusb.c", "common/tusb_fifo.c", "device/usbd.c", "device/usbd_control.c", "class/hid/hid_device.c", "class/msc/msc_device.c"]]
...
def build(directory, sanitize=True, coverage=False):
    ...
    command = [os.environ.get("CC", "cc"), "-std=c11", "-Wall", "-Wextra", "-Werror",
               ...
               str(ROOT / "tests/usb_stack/test_usb_stack.c"), str(ROOT / "src/usb_descriptors.c"), str(ROOT / "src/usb.c")]
