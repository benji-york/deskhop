...
The accepted version bump and hardware record were committed as `40d4c8a`;
the pinned host updater and its safety tests were committed separately as
`b1d1c8b`. Integration used the clean
`/private/tmp/deskhop-merge-v106.jQ2eSb` checkout. Documentation/test conflicts
were resolved without changing either the accepted firmware inputs or the
hardware-exercised updater implementation/tests. Independent comparison
confirmed both preservation boundaries.

All 42 fast-tier steps passed (76.277 seconds of recorded command durations),
including all 118 updater tests, native/storage/TinyUSB boundaries and paired
production scenarios. Evidence: that checkout's `build/tests/results-fast.json`.
A fresh ARM configure/build also passed. The rebuilt BIN and UF2 are
byte-for-byte identical to the deployed frozen v0.106 candidate:

- BIN SHA256: `55c78043fbdcf445a9807caef67ca1feff908af5b74c7cf462406639c0672e2a`.
- UF2 SHA256: `011f7a02a1b4a2648d2d7bacff1cbbdf232a0f6afbada6d273ba231c864cd83d`.

No new hardware operation accompanied this merge. User acceptance authorizes
publication of the operationally tested implementation, not a claim that the
batch performance objective has been met. The original failed diagnostic run
and separate successful verification remain distinct records.
