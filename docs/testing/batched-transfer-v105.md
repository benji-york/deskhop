...
This record preserves the v0.105 implementation and historical deployment
attempts. Current status: the same firmware with a version-only v0.106 bump
propagated successfully, passed fresh both-board verification and was accepted
by Benji, who authorized merging it into `main`. Actual batch use and speedup
remain unproven. See [the v0.106 record](batched-transfer-hardware-v106.md) for
current evidence; it supersedes older status/acceptance claims below.
